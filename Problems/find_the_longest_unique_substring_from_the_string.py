  ...
  def find_longest_unique_string(self, given_str):
    ...
    for i in range(1, len(given_str)):
      if given_str[i] not in pos:
        ...
        # Check that repeated value's position is greater than or equal to
        # starting point.
    ...
    if max_length < i - st:
      max_length = i - st + 1 # Need to increment by one for taking the last element "abcabcdefabefabcdefgh"
      start = st
